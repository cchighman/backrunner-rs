...
pub async fn cyclic_order(
    ...
    let a1_b3 = evaluate_symbols(
        left_cyclic_symbol(crypto_path[0].left_symbol()),
        right_cyclic_symbol(crypto_path[2].right_symbol()),
        None,
    );

    let b1_b3 = evaluate_symbols(
        left_cyclic_symbol(crypto_path[0].right_symbol()),
        right_cyclic_symbol(crypto_path[2].right_symbol()),
        None,
    );
    let b2_b3 = evaluate_symbols(
        left_cyclic_symbol(crypto_path[1].right_symbol()),
        right_cyclic_symbol(crypto_path[2].right_symbol()),
        None,
    );
    let a2_b3 = evaluate_symbols(
        left_cyclic_symbol(crypto_path[1].left_symbol()),
        right_cyclic_symbol(crypto_path[2].right_symbol()),
        None,
    );

    let b1_a2 = evaluate_symbols(
        left_cyclic_symbol(crypto_path[0].right_symbol()),
        right_cyclic_symbol(crypto_path[1].left_symbol()),
        None,
    );
    let b2_a3 = evaluate_symbols(
        left_cyclic_symbol(crypto_path[1].right_symbol()),
        right_cyclic_symbol(crypto_path[2].left_symbol()),
        None,
    );
    let a1_a2 = evaluate_symbols(
        left_cyclic_symbol(crypto_path[0].left_symbol()),
        right_cyclic_symbol(crypto_path[1].left_symbol()),
        None,
    );
    let b1_b2 = evaluate_symbols(
        left_cyclic_symbol(crypto_path[0].right_symbol()),
        right_cyclic_symbol(crypto_path[1].right_symbol()),
        None,
    );
    let a2_a3 = evaluate_symbols(
        left_cyclic_symbol(crypto_path[1].left_symbol()),
        right_cyclic_symbol(crypto_path[2].left_symbol()),
        None,
    );
    let a1_b2 = evaluate_symbols(
        left_cyclic_symbol(crypto_path[0].left_symbol()),
        right_cyclic_symbol(crypto_path[1].right_symbol()),
        None,
    );
    let b1_a3 = evaluate_symbols(
        left_cyclic_symbol(crypto_path[0].right_symbol()),
        right_cyclic_symbol(crypto_path[2].left_symbol()),
        None,
    );
    let a1_a3 = evaluate_symbols(
        left_cyclic_symbol(crypto_path[0].left_symbol()),
        right_cyclic_symbol(crypto_path[2].left_symbol()),
        None,
    );
    ...
    // Orientation of whether structured left or right depends on scenario
    ...
    */
    ...
    if token_b1.clone().unwrap().symbol() == token_a2.clone().unwrap().symbol()
        && token_b2.clone().unwrap().symbol() == token_a3.clone().unwrap().symbol()
    {
    ...
    pub async fn calculate(sequence: Arc<ThreePathSequence>, pending: bool) {
        ...
        if !result.is_none() {
            ...
            if profit > U256::zero() {
                ...
                //28161198633 376.70899132191087
                //430425983304397055525.5450621
                // Does the amounts in/out support our delta calculations?

                let delta_a_amt_out = SequenceToken::get_amount_out(
                    delta_a,
                    sequence.a1().pending_reserve(),
                    sequence.b1().pending_reserve(),
                );

                if delta_a_amt_out.is_some() {
                    let delta_a_amt_in = SequenceToken::get_amount_in(
                        delta_b,
                        sequence.a1().pending_reserve(),
                        sequence.b1().pending_reserve(),
                    );

                    let delta_b_amt_out = SequenceToken::get_amount_out(
                        delta_c,
                        sequence.a2().pending_reserve(),
                        sequence.b2().pending_reserve(),
                    );
                    if delta_b_amt_out.is_some() {
                        let delta_b_amt_in = SequenceToken::get_amount_in(
                            delta_b,
                            sequence.a2().pending_reserve(),
                            sequence.b2().pending_reserve(),
                        );

                        let delta_c_amt_out = SequenceToken::get_amount_out(
                            delta_a_prime,
                            sequence.a3().pending_reserve(),
                            sequence.b3().pending_reserve(),
                        );
                        if delta_c_amt_out.is_some() {
                            let delta_c_amt_in = SequenceToken::get_amount_in(
                                delta_c_amt_out.unwrap(),
                                sequence.a3().pending_reserve(),
                                sequence.b3().pending_reserve(),
                            );

                            if delta_a_amt_out.is_none()
                                || delta_a_amt_in.is_none()
                                || delta_b_amt_out.is_none()
                                || delta_b_amt_in.is_none()
                                || delta_c_amt_out.is_none()
                                || delta_c_amt_in.is_none()
                            {
                                return;
                            }

                            let dec = (sequence.a1().decimal() - sequence.b1().decimal()).abs();
                            let delta_a_amt_out_dec =
                                ratio_to_dec(delta_a_amt_out.unwrap().to_big_rational()).div(
                                    BigDecimal::from_u128(10_u128.pow(dec.try_into().unwrap()))
                                        .unwrap(),
                                );

                            let delta_a_amt_in_dec =
                                ratio_to_dec(delta_a_amt_in.unwrap().to_big_rational()).div(
                                    BigDecimal::from_u128(10_u128.pow(dec.try_into().unwrap()))
                                        .unwrap(),
                                );

                            let delta_b_amt_out_dec =
                                ratio_to_dec(delta_a_amt_in.unwrap().to_big_rational()).div(
                                    BigDecimal::from_u128(
                                        10_u128.pow(sequence.a2().decimal().try_into().unwrap()),
                                    )
                                    .unwrap(),
                                );

                            let delta_b_amt_in_dec =
                                ratio_to_dec(delta_b_amt_in.unwrap().to_big_rational()).div(
                                    BigDecimal::from_u128(
                                        10_u128.pow(sequence.b2().decimal().try_into().unwrap()),
                                    )
                                    .unwrap(),
                                );

                            let delta_c_amt_out_dec =
                                ratio_to_dec(delta_b_amt_in.unwrap().to_big_rational()).div(
                                    BigDecimal::from_u128(
                                        10_u128.pow(sequence.a3().decimal().try_into().unwrap()),
                                    )
                                    .unwrap(),
                                );

                            let delta_c_amt_in_dec =
                                ratio_to_dec(delta_c_amt_in.unwrap().to_big_rational()).div(
                                    BigDecimal::from_u128(
                                        10_u128.pow(sequence.b3().decimal().try_into().unwrap()),
                                    )
                                    .unwrap(),
                                );

                            let t1_initial_price = sequence
                                .a1()
                                .pending_reserve()
                                .div(sequence.b1().pending_reserve());
                            let t1_after_price = sequence.a1().pending_reserve().add(delta_a).div(
                                sequence
                                    .b1()
                                    .pending_reserve()
                                    .sub(delta_a_amt_out.unwrap()),
                            );

                            let t2_initial_price = sequence
                                .a2()
                                .pending_reserve()
                                .mul(sequence.b2().pending_reserve());
                            let t2_after_price = sequence.a2().pending_reserve().add(delta_b).div(
                                sequence
                                    .b2()
                                    .pending_reserve()
                                    .sub(delta_b_amt_out.unwrap()),
                            );

                            let t3_initial_price = sequence
                                .a3()
                                .pending_reserve()
                                .mul(sequence.b3().pending_reserve());
                            let t3_after_price = sequence.a3().pending_reserve().add(delta_c).div(
                                sequence
                                    .b3()
                                    .pending_reserve()
                                    .sub(delta_c_amt_out.unwrap()),
                            );
                        ...
                        \t\tdelta_a_prime: {:.25}\t\t amount_in: {:.25}\t reserves: {}",
                                ...
                                &delta_b.to_string(),
                                &delta_a_amt_in.unwrap().to_string(),
                                &sequence.a1().token.pair.token1.reserve.to_string(),
                                //           trade1_arb.calldata().await.unwrap(),
                                //         trade1.calldata().await.unwrap(),
                                // Trade 2
                                &delta_b_amt_out_dec,
                                &sequence.a2().symbol(),
                                &delta_b_amt_in_dec,
                                ...
                                //     trade2_arb.calldata().await.unwrap(),
                                //     trade2.calldata().await.unwrap(),
                                ...
                                &delta_c_amt_in.unwrap(),
                                &sequence.b3().pending_reserve(),
                                //             trade3_arb.calldata().await.unwrap(),
                                //             trade3.calldata().await.unwrap(),
                            );
                            // }

                            let flash_token =
                                IERC20::new(*sequence.a1().id(), mainnet::client.clone());
                            let flash_repayment = flash_token.transfer(
                                *sequence.a1().pair_id(),
                                delta_a.mul(U256::from(997_i16)),
                            );
...
pub async fn test_calculate() {
  ...
  Pair1: 0x5769e99b0c5dc4f8a1657d9f7c4ef7e7b015583d-
    ...
    */
    ...
    let d_c = U256::from(34250_u128);
    let d_a_prime = U256::from(24250_u128);
    ...
    if !result.is_none() {
        ...
        let c1 = a1 * b1;
        let eq2 = (a2 + nine_seven * d_b) * (b2.checked_sub(d_c).unwrap());
        let c2 = a2 * b2;
        let eq3 = (a3 + nine_seven * d_c) * (b3.checked_sub(d_a_prime).unwrap());
        let c3 = a3 * b3;
        println!(
            "there - eq1: {} c1: {} eq2: {} c2: {} eq3: {} c3: {}",
            eq1, c1, eq2, c2, eq3, c3
        );

        let eq1 = (a1 + nine_seven * delta_a) * (b1.checked_sub(delta_b).unwrap());
        let c1 = a1 * b1;
        let eq2 = (a2 + nine_seven * delta_b) * (b2.checked_sub(delta_c).unwrap());
        let c2 = a2 * b2;
        let eq3 = (a3 + nine_seven * delta_c) * (b3.checked_sub(delta_a_prime).unwrap());
        let c3 = a3 * b3;
        println!(
            "mine - eq1: {} c1: {} eq2: {} c2: {} eq3: {} c3: {}",
            eq1, c1, eq2, c2, eq3, c3
        );
        ...
        let delta_a_amt_out_dec =
            ratio_to_dec(delta_a_amt_out.unwrap().to_big_rational()).div(
                BigDecimal::from_u128(
                    10_u128.pow(dec.try_into().unwrap()),
                )
                .unwrap(),
            );
