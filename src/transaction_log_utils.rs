...
use ethabi::{RawLog, Event, EventParam, ParamType};
...
pub static topic_map: Lazy<HashMap<H256,(&str, &str)>> = Lazy::new(|| -> HashMap<TxHash, (&str, &str)> { HashMap::from([
    (H256::from_str("0xdccd412f0b1252819cb1fd330b93224ca42612892bb3f4f789976e6d81936496").unwrap(),("Burn","Burn(address,uint256,uint256,address)")),
    (H256::from_str("0x4c209b5fc8ad50758f13e2e1088ba56a560dff690a1c6fef26394f4c03821c4f").unwrap(),("Mint","Mint(address,uint256,uint256)")),
    (H256::from_str("0xd78ad95fa46c994b6551d0da85fc275fe613ce37657fb8d5e3d130840159d822").unwrap(),("Swap","Swap(address,uint256,uint256,uint256,uint256,address)")),
    (H256::from_str("0x1c411e9a96e071241c2f21f7726b17ae89e3cab4c78be50e062b03a9fffbbad1").unwrap(),("Sync","Sync(uint112,uint112)"))]) });

    use ethers::providers::{MockProvider, Provider};
    use ethers::prelude::*;


    pub async fn decode_uniswap_event(topic: (&str,&str), topics:  Vec<H256>, decoded: Bytes)->ethabi::Log {
    ...
    pub fn decode_event_tx(method: &str, signature: &str, topics: Vec<H256>, payload: Bytes)->ethabi::Log {
        /* 
        abigen!(
            Test,
             r#"[
         ...
         */  
         let raw_log = RawLog { topics: topics.clone(), data: payload.to_vec() };
         let mut event = "";
        ...
        let log_event = Event{name: event.to_string(), inputs: params, anonymous: false};
